...
public abstract class ImageWorker {
	...
	private boolean isLastPinPlotting;

	public boolean isLastPinPlotting() {
		return isLastPinPlotting;
	}

	public void setLastPinPlotting(boolean isLastPinPlotting) {
		this.isLastPinPlotting = isLastPinPlotting;
	}
	 ...
	 */
	/*
	 * public void loadImage(int num, ImageView imageView) { if
	 * (mImageWorkerAdapter != null) {
	 * loadImage(mImageWorkerAdapter.getItem(num), imageView); } else { throw
	 * new NullPointerException( "Data not set, must call setAdapter() first.");
	 * } }
	 */
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
		...
		private final WeakReference<ViewFlipper> viewFlipperWeakReference;

		public BitmapWorkerTask(ImageView imageView, ViewFlipper viewFlipper) {
			imageViewReference = new WeakReference<ImageView>(imageView);
			viewFlipperWeakReference = new WeakReference<ViewFlipper>(
					viewFlipper);
		}
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			final ViewFlipper viewFlipper = getAttachedViewFlipper();
			if (bitmap != null && imageView != null && viewFlipper != null) {
				setImageBitmap(imageView, bitmap, viewFlipper, data);
			}
		}
		 ...
		 * otherwise.
		 */
		private ViewFlipper getAttachedViewFlipper() {
			return viewFlipperWeakReference.get();
			/*
	...
	private void setImageBitmap(ImageView imageView, Bitmap bitmap,
			ViewFlipper viewFlipper, Object data) {
		if (mImageCache != null) {
			imageView.setImageBitmap(mImageCache.getBitmapFromMemCache(String
					.valueOf(data)));
			viewFlipper.setDisplayedChild(1);
		} else if (bitmap != null) {// TODO: Remove this else
			imageView.setImageBitmap(bitmap);
			viewFlipper.setDisplayedChild(1);
		} else {
			viewFlipper.setDisplayedChild(1);
	 ...
	 */
	/*
	 * public void setAdapter(ImageWorkerAdapter adapter) { mImageWorkerAdapter
	 * = adapter; }
	 *//**
	 ...
	 */
	/*
	 * public ImageWorkerAdapter getAdapter() { return mImageWorkerAdapter; }
	 *//**
	 * A very simple adapter for use with ImageWorker class and subclasses.
	 */
	/*
	 * public static abstract class ImageWorkerAdapter { public abstract Object
	 * getItem(int num);
	 * 
