...
public abstract class DraggableDrawer extends MenuDrawer
	{
		 ...
		 */
		private static final String STATE_MENU_VISIBLE = "com.qonect.utility.navigationdrawer.menudrawer.MenuDrawer.menuVisible";
		...
		/**
		 * Default delay from {@link #peekDrawer()} is called until first
		 * animation is run.
		 */
		...
		/**
		 * Distance in dp from closed position from where the drawer is
		 * considered closed with regards to touch events.
		 */
		...
		/**
		 * Velocity tracker used when animating the drawer open/closed after a
		 * drag.
		 */
		...
		/**
		 * Distance in px from closed position from where the drawer is
		 * considered closed with regards to touch events.
		 */
		protected int mCloseEnough;

		/**
		 * Runnable used for first call to {@link #startPeek()} after
		 * {@link #peekDrawer()} has been called.
		 */
		 ...
		 * 
		 * @param offsetPixels
		 *            The number of pixels to offset the content by.
		 */
		...
		/**
		 * Called when the number of pixels the content should be offset by has
		 * changed.
		 * 
		 * @param offsetPixels
		 *            The number of pixels to offset the content by.
		 ...
		 * {@link android.view.View#LAYER_TYPE_HARDWARE}, this will set it to
		 ...
		 * 
		 * @param position
		 *            The position the content is moved to.
		 * @param velocity
		...
		/**
		 * Callback when each frame in the peek drawer animation should be
		 * drawn.
		 */
		 ...
		 * @param ev
		 *            The motion event.
		 * @return True if the touch event occurred over the content, false
		 *         otherwise.
		 */
		 ...
		 * @param ev
		 *            The motion event.
		 * @return True if dragging the content should be allowed, false
		 *         otherwise.
		 */
		...
		/**
		 * Called when a move event has happened while dragging the content is
		 * in progress.
		 * 
		 * @param dx
		 *            The X difference between the last motion event and the
		 *            current motion event.
		 */
		protected abstract void onMoveEvent(float dx);

		/**
		 * Called when {@link android.view.MotionEvent#ACTION_UP} of
		 * {@link android.view.MotionEvent#ACTION_CANCEL} is delivered to
		 * {@link com.ih.utility.navigationdrawer.menudrawer.MenuDrawer#onTouchEvent(android.view.MotionEvent)}
		 * .
		 * 
		 * @param ev
		 *            The motion event.
		...
		protected void dispatchDraw(Canvas canvas)
			{
				...
				if (mActiveIndicator != null)
					drawIndicator(canvas, offsetPixels);
			}
